...
export interface RecipeEntry {
  ...
  /** When element was used for mouse_click/double_click/scroll — coordinates and element description. */
  click?: { x: number; y: number; element?: string };
}

export interface UserInjection {
  text: string;
  afterStepIndex: number; // show "User said" between step N and N+1
}
...
/** When placeAfterNextStep is true (e.g. user replied during ask_user), show "User said" below the next step (ask_user). */
...
export function finalize(isSuccess: boolean, assessment = "", _clarification = ""): void {
  ensureRecipe();
  if (!recipe) return;
...
/** Escape content for markdown fenced block (use ~~~ to avoid ``` in content) */
...
export async function writeRecipeToTempFile(): Promise<string> {
  const html = generateHtml(true);
  const path = join(await import("node:os").then((os) => os.tmpdir()), `houston-recipe-${Date.now()}.html`);
  writeFileSync(path, html, "utf-8");
  return path;
}

/** Generate index.html that loads RECIPE.md and renders it as formatted markdown. */
