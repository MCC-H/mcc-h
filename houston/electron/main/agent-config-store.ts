/**
 * Agent-only config storage. Uses UUIDs as keys. Each config has detailed_description and value.
 */
...
const AGENT_CONFIG_PATH = join(HOUSTON_DIR, "agent-config.json");
...
function loadAgentConfig(): AgentConfigEntry[] {
  try {
    ...
    console.warn("[Houston] Agent config load failed:", err);
  }
  return [];
}
...
function saveAgentConfig(items: AgentConfigEntry[]): void {
  try {
    ...
    console.error("[Houston] Failed to save agent config:", err);
  }
}
