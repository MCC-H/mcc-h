/**
 * Agent-only secrets storage. Uses UUIDs as keys. Each secret has detailed_description,
 * first_factor, first_factor_type, and value.
 */
...
const SECRETS_PATH = join(HOUSTON_DIR, "secrets.json");
...
function loadSecrets(): SecretEntry[] {
  try {
    if (existsSync(SECRETS_PATH)) {
      ...
      // Old format: { "key": "value" } - migrate in memory
    ...
    console.warn("[Houston] Secrets load failed:", err);
  }
  return [];
}
...
function saveSecrets(items: SecretEntry[]): void {
  try {
    ...
    console.error("[Houston] Failed to save secrets:", err);
  }
}
